...
e.preventDefault()
try {
await fetch(`${API_BASE}/products`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
...
e.preventDefault()
try {
await fetch(`${API_BASE}/transactions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(trans) })
...
<section style={{ marginBottom: 20 }}> <h2>Products</h2>
{loading ? <div>Loading...</div> : (
<table border="1" cellPadding="8" style={{ borderCollapse: 'collapse' }}> <thead><tr><th>ID</th><th>SKU</th><th>Name</th><th>Stock</th><th>Reorder</th></tr></thead> <tbody>
{products.map(p => ( <tr key={p.id}><td>{p.id}</td><td>{p.sku}</td><td>{p.name}</td><td>{p.currentStock}</td><td>{p.reorderLevel}</td></tr>
))} </tbody> </table>
)} </section>
